...
from mv3d.baselines.pointmvsnet.networks import *
from mv3d.baselines.pointmvsnet.functions.functions import get_pixel_grids, get_propability_map
from mv3d.baselines.pointmvsnet.utils.feature_fetcher import FeatureFetcher
from mv3d.baselines.pointmvsnet.utils.torch_utils import get_knn_3d
from mv3d.baselines.pointmvsnet.nn.mlp import SharedMLP
...
class PointMVSNet(nn.Module):
    ...
    def forward(self, data_batch, img_scales, inter_scales, isFlow, isTest=False):
        ...
        cam_intrinsic = cam_params_list[:, :, 1, :3, :3].clone()                # 448x640
        cam_intrinsic[:, :, :2, :3] = cam_intrinsic[:, :, :2, :3] / 8.0         # 56x80
        # cam_intrinsic[:, :, :2, :3] = cam_intrinsic[:, :, :2, :3] / 2.0         # 224x320
        # if isTest:      # TODO: why divide by an additional 4 here?
        #     cam_intrinsic[:, :, :2, :3] = cam_intrinsic[:, :, :2, :3] / 4.0     # 56x80 why???
        ...
        # pred_depth_img += torch.normal(0., 0.30, size=pred_depth_img.shape).type_as(pred_depth_img)
    ...
    def train(self, mode=True):
        ...
        """
        super(PointMVSNet, self).train(mode)
...
class PointMVSNetLoss(nn.Module):
    ...
    def forward(self, preds, labels, isFlow):
        ...
        coarse_loss = self.maeloss(coarse_depth_map, resize_gt_depth, depth_intervals)
        ...
        if isFlow:
            ...
            flow1_loss = self.maeloss(flow1, resize_gt_depth, 0.75 * depth_intervals)
            losses["flow1_loss"] = flow1_loss

            flow2 = preds["flow2"]
            resize_gt_depth = F.interpolate(gt_depth_img, (flow2.shape[2], flow2.shape[3]))
            flow2_loss = self.maeloss(flow2, resize_gt_depth, 0.375 * depth_intervals)
...
if __name__ == "__main__":
    ...
    pmvs = PointMVSNet()
    loss_fn = PointMVSNetLoss(0.5)
    metric_fn = PointMVSNetMetric(0.5)

...
def norm_image_tensor(img):
    """ normalize image using tensor ops """
    img = img.float()
    var = img.var(dim=(2, 3), keepdims=True)
    mean = img.mean(dim=(2, 3), keepdims=True)
    return (img - mean) / (torch.sqrt(var) + 1e-7)

...
from pointmvsnet.model import PointMVSNet, PointMVSNetLoss
from pointmvsnet.utils.preprocess import norm_image_tensor


class PointMVSNetPLModel(pl.LightningModule):
    ...
    def load_pretrained_pointmvsnet(self, weights_folder=os.path.join(os.path.dirname(__file__), 'pmvs_weights')):
        ckpt = torch.load(os.path.join(weights_folder, 'pretrained.pth'))
        state_dict = {k[7:]: ckpt['model'][k] for k in ckpt['model'].keys()}
        self.pointmvsnet.load_state_dict(state_dict, True)
    ...
    def forward(self, batch, img_scales, inter_scales, depth_start, depth_interval, n_depth, move_to_cpu=False):
        normed_images = norm_image_tensor(batch.images)
        ...
        for i in range(n_ref_imgs):
            ...
            img_list = torch.cat((normed_images[r_idx].unsqueeze(0), normed_images[s_idx]), dim=0).unsqueeze(0)
            ...
            preds = self.pointmvsnet(data_batch, img_scales, inter_scales, isFlow=True, isTest=not self.training)
            if move_to_cpu:
                for k in preds.keys():
